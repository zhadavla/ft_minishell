...
/*   Updated: 2023/08/15 21:34:11 by vzhadan          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		enum e_quote *q_s)
{
	...
	if ((*q_s == IN_QUOTE2 && token_type == DOUBLE_QUOTE)
		|| (*q_s == IN_QUOTE1 && token_type == SINGLE_QUOTE))
...
t_token	*apply_lexer(char *str)
{
	...
	while (str[++i])
	{
		...
		{
			if (len > 0)
				t_add(&head, new_token(ft_substr(str, i - len, len),
						len, WORD, q_s));
